module Learning
  module ChapterLearningsHelper
    def unit_exam_link(unit_learning)
      if unit_learning.ready_for_exam?
        if unit_learning.unfinished_exam
          link_to '继续上次练习', learning_unit_exam_path(unit_learning.unfinished_exam), :class => 'btn btn-sm btn-primary'
        else
          if unit_learning.exam_limited?
            raw "<span class='btn btn-sm btn-primary'>开始练习</span>"
          else
            link_to '开始练习', learning_unit_learning_unit_exams_path(unit_learning), :method => :post, :class => "btn btn-sm btn-primary"
          end
        end
      end
    end
    ...
    def mode_switch_link(video_watching)
      ...
      link_to text, learning_video_watching_path(video_watching, :with_comment => !video_watching.with_comment?)
    end
  end
end
