    ...
    def unit_exam_link(unit_learning)
      if unit_learning.ready_for_exam?
        if unit_learning.unfinished_exam
          link_to '继续测试', unit_exam_path(unit_learning.unfinished_exam), :class => 'start-exam'
        else
          link_to '开始测试', unit_learning_unit_exams_path(unit_learning), :method => :post, :class => 'start-exam'
        end
      end
    end
